...
def test_v49_path_length_limit():
    original = agent.MAX_LIST_PATH_CHARS
    name = "v49_long_filename.txt"
    ...
    try:
        agent.MAX_LIST_PATH_CHARS = 1

        expect_blocked(
            "V49 List Path Length Limit",
            lambda: agent.list_files(),
        )

    finally:
        agent.MAX_LIST_PATH_CHARS = original
