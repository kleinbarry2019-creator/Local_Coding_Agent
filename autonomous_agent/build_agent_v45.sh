...
echo "=========================================="
echo " SAFE AGENT V45 - SYMLINK + POLICY LOCK"
echo "=========================================="
echo "Project: $ROOT"
...
# ---------------------------------------------------------
# Alte Audit-Historie sauber archivieren
# ---------------------------------------------------------

if [[ -f audit_log.jsonl ]]; then
    if [[ -s audit_log.jsonl ]]; then
        backup="logs/audit_pre_v45_$(date +%Y%m%d_%H%M%S).jsonl"
...
def reject_symlink_components(target):
    ...
    # Vor jedem Dateizugriff werden alle bereits existierenden
    # Komponenten geprüft. So werden versteckte Symlink-Ziele
...
def resolve_runtime_path(path_text):
    ...
    # Unterstützt die vom Modell häufig verwendete
    # /workspace/...-Notation, ohne Hostpfade zu übernehmen.
...
def write_file(path, content):
    ...
    # Nach dem Schreiben erneut prüfen.
    reject_symlink_components(
        target
    )
...
def main():
    if len(sys.argv) < 2:
        print(
            'Usage: python3 safe_agent_v45.py "AUFGABE"'
        )
        raise SystemExit(2)

    print("=== SAFE AGENT V45 ===")
    print("Project:", ROOT)
...
def test_policy_lock():
    ...
    assert verified == agent.DEFAULT_POLICY
    assert lock.digest == agent.policy_digest(
        agent.DEFAULT_POLICY
    )


def test_policy_lock_snapshot_isolation():
    original = dict(agent.DEFAULT_POLICY)
    ...
    # Die bereits erzeugte Mission darf von späteren
    # Änderungen am Ursprungsobjekt nicht beeinflusst werden.
    verified = lock.verify()
...
cat > tools/test_all_v45.sh <<'BASH2'
#!/usr/bin/env bash
set -Eeuo pipefail
...
python3 -m py_compile safe_agent_v45.py

python3 tests/test_v45.py
...
echo
echo "V45 TESTS PASSED"
BASH2

chmod +x tools/test_all_v45.sh


cat > tools/test_end_to_end_v45.sh <<'BASH3'
#!/usr/bin/env bash
set -Eeuo pipefail
...
chmod +x tools/test_end_to_end_v45.sh


cat > BUILD_INFO_V45.txt <<EOF
SAFE AGENT V45
...
echo
echo "=== V45 END-TO-END ==="
./tools/test_end_to_end_v45.sh

echo
echo "=========================================="
echo " SAFE AGENT V45 READY"
echo "=========================================="
