...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; } body { margin:0; min-height:100vh; font:16px/1.5 system-ui,sans-serif; color:var(--ink); background:radial-gradient(circle at top right,#183b55 0,var(--bg) 42%); }
    ...
    textarea, select, input[type="text"], input[type="password"], input[type="number"] { width:100%; border:1px solid var(--line); border-radius:12px; padding:12px; color:var(--ink); background:var(--field); font:inherit; }
...
<main>
  ...
  <section class="panel"><div class="label">Einstellungen &amp; Profil</div><form id="preferences-form"><div class="settings-grid"><div class="setting"><label for="theme">Erscheinungsbild</label><select id="theme"><option value="system">System</option><option value="light">Hell</option><option value="dark">Dunkel</option></select></div><div class="setting"><label for="response-style">Antwortstil</label><select id="response-style"><option value="concise">Kurz</option><option value="balanced">Ausgewogen</option><option value="detailed">Ausführlich</option></select></div><div class="setting"><label for="knowledge-level">Kenntnisstand</label><select id="knowledge-level"><option value="beginner">Anfänger</option><option value="hobbyist">Hobbyist</option><option value="advanced">Fortgeschritten</option><option value="expert">Erweitert fortgeschritten</option><option value="developer">Developer</option></select></div><div class="setting"><label for="nickname">Nickname/Alias</label><input id="nickname" type="text" maxlength="160" autocomplete="nickname"></div><div class="setting"><label for="interests">Interessen (optional)</label><input id="interests" type="text" maxlength="160"></div><div class="setting"><label for="age">Alter (optional)</label><input id="age" type="number" min="1" max="125" inputmode="numeric"></div><div class="setting"><label for="occupation">Beruf (optional)</label><input id="occupation" type="text" maxlength="160"></div><div class="setting"><label for="gender-identity">Geschlechtsidentität (optional)</label><input id="gender-identity" type="text" maxlength="160"></div><div class="setting"><label for="pronoun-mode">Anrede</label><select id="pronoun-mode"><option value="neutral">Neutral</option><option value="gendered">Geschlechtsspezifisch</option><option value="custom">Eigene Pronomen</option></select></div><div class="setting"><label for="pronouns">Pronomen (optional)</label><input id="pronouns" type="text" maxlength="160"></div><label class="check"><input id="simple-language" type="checkbox"> Einfache Sprache</label><label class="check"><input id="gendered-language" type="checkbox"> Gendern verwenden</label><label class="check"><input id="allow-network" type="checkbox"> Online-Recherche erlauben</label><label class="check"><input id="store-personalization" type="checkbox"> Profil lokal speichern</label></div><button id="save-preferences" type="submit">Einstellungen speichern</button><div id="preferences-state"></div></form></section>
  <section class="panel"><div class="label">Benutzerkonto</div><p>Das Konto bleibt lokal. Für die Synchronisierung werden keine E-Mail- oder Adressdaten benötigt.</p><form id="account-form"><div class="settings-grid"><div class="setting"><label for="account-username">Benutzername</label><input id="account-username" type="text" minlength="3" maxlength="32" autocomplete="username" required></div><div class="setting"><label for="account-password">Passwort</label><input id="account-password" type="password" minlength="10" maxlength="256" autocomplete="new-password" required></div><div class="setting"><label for="account-question">Sicherheitsfrage (optional)</label><input id="account-question" type="text" maxlength="200"></div><div class="setting"><label for="account-answer">Antwort (optional)</label><input id="account-answer" type="password" maxlength="200" autocomplete="off"></div><div class="setting"><label for="account-recovery-answer">Antwort zur Wiederherstellung</label><input id="account-recovery-answer" type="password" maxlength="200" autocomplete="off"></div><div class="setting"><label for="account-new-password">Neues Passwort</label><input id="account-new-password" type="password" minlength="10" maxlength="256" autocomplete="new-password"></div></div><button id="account-create" type="button">Konto erstellen</button><button id="account-login" type="button">Anmelden</button><button id="account-reset" type="button">Passwort zurücksetzen</button><div id="account-state"></div></form><div id="account-list"></div></section>
  <div class="grid"><section class="panel"><div class="label">Aktueller Status</div><div id="current">Noch kein Auftrag gestartet.</div><button id="undo" type="button" disabled>Letzten Schritt rückgängig</button><div id="undo-state"></div></section><section class="panel"><div class="label">Verlauf</div><div id="history">Keine Aufträge in dieser Sitzung.</div></section><section class="panel"><div class="label">Wissen &amp; Verbesserungen</div><div id="learning">Lernstatus wird geladen …</div><button id="research" type="button">Nach vertrauenswürdigen Quellen suchen</button><div id="research-state"></div></section></div>
</main>
<script>
const TOKEN = __ACB_TOKEN__; const form = document.getElementById('task-form'); const goal = document.getElementById('goal'); const submit = document.getElementById('submit'); const state = document.getElementById('state'); const current = document.getElementById('current'); const history = document.getElementById('history'); const learning = document.getElementById('learning'); const research = document.getElementById('research'); const researchState = document.getElementById('research-state'); const undo = document.getElementById('undo'); const undoState = document.getElementById('undo-state'); let activeSession = null; const preferencesForm = document.getElementById('preferences-form'); const theme = document.getElementById('theme'); const responseStyle = document.getElementById('response-style'); const knowledgeLevel = document.getElementById('knowledge-level'); const nickname = document.getElementById('nickname'); const interests = document.getElementById('interests'); const age = document.getElementById('age'); const occupation = document.getElementById('occupation'); const genderIdentity = document.getElementById('gender-identity'); const pronounMode = document.getElementById('pronoun-mode'); const pronouns = document.getElementById('pronouns'); const simpleLanguage = document.getElementById('simple-language'); const genderedLanguage = document.getElementById('gendered-language'); const allowNetwork = document.getElementById('allow-network'); const storePersonalization = document.getElementById('store-personalization'); const savePreferences = document.getElementById('save-preferences'); const preferencesState = document.getElementById('preferences-state'); const accountUsername = document.getElementById('account-username'); const accountPassword = document.getElementById('account-password'); const accountQuestion = document.getElementById('account-question'); const accountAnswer = document.getElementById('account-answer'); const accountRecoveryAnswer = document.getElementById('account-recovery-answer'); const accountNewPassword = document.getElementById('account-new-password'); const accountCreate = document.getElementById('account-create'); const accountLogin = document.getElementById('account-login'); const accountReset = document.getElementById('account-reset'); const accountState = document.getElementById('account-state'); const accountList = document.getElementById('account-list');
...
async function refreshPreferences() { try { const data = await request('/api/preferences',{headers:{}}); theme.value = data.theme || 'system'; responseStyle.value = data.response_style || 'balanced'; knowledgeLevel.value = data.knowledge_level || 'hobbyist'; nickname.value = data.nickname || ''; interests.value = data.interests || ''; age.value = data.age ?? ''; occupation.value = data.occupation || ''; genderIdentity.value = data.gender_identity || ''; pronounMode.value = data.pronoun_mode || 'neutral'; pronouns.value = data.pronouns || ''; simpleLanguage.checked = Boolean(data.simple_language); genderedLanguage.checked = Boolean(data.gendered_language); allowNetwork.checked = Boolean(data.allow_network_research); storePersonalization.checked = Boolean(data.store_personalization); applyTheme(theme.value); } catch (error) { preferencesState.textContent = 'Einstellungen nicht verfügbar.'; preferencesState.className='error'; } }
async function refreshAccount() { try { const data = await request('/api/account',{headers:{}}); const accounts = data.accounts || []; accountList.textContent = accounts.length ? `Lokale Konten: ${accounts.map(item => esc(item.username)).join(', ')}` : 'Noch kein lokales Konto angelegt.'; } catch (error) { accountList.textContent = 'Kontostatus nicht verfügbar.'; } }
async function accountAction(action) { accountCreate.disabled=true; accountLogin.disabled=true; accountState.textContent = action === 'create' ? 'Konto wird erstellt …' : 'Anmeldung wird geprüft …'; try { const result=await request('/api/account',{method:'POST',body:JSON.stringify({action,username:accountUsername.value,password:accountPassword.value,security_question:accountQuestion.value,security_answer:accountAnswer.value})}); accountState.textContent = action === 'create' ? `Konto ${esc(result.username || '')} erstellt.` : `Angemeldet als ${esc(result.username || '')}.`; accountState.className='success'; accountPassword.value=''; accountAnswer.value=''; await refreshAccount(); } catch(error) { accountState.textContent=error.message; accountState.className='error'; } finally { accountCreate.disabled=false; accountLogin.disabled=false; } }
async function resetPassword() { accountReset.disabled=true; accountState.textContent='Passwort wird zurückgesetzt …'; try { const result=await request('/api/account',{method:'POST',body:JSON.stringify({action:'reset-password',username:accountUsername.value,security_answer:accountRecoveryAnswer.value,new_password:accountNewPassword.value})}); accountState.textContent = result.username ? `Passwort für ${esc(result.username)} geändert.` : 'Passwort geändert.'; accountState.className='success'; accountRecoveryAnswer.value=''; accountNewPassword.value=''; accountPassword.value=''; } catch(error) { accountState.textContent=error.message; accountState.className='error'; } finally { accountReset.disabled=false; } }
...
preferencesForm.addEventListener('submit', async event => { event.preventDefault(); savePreferences.disabled=true; preferencesState.textContent='Speichere …'; try { const ageValue = age.value.trim() === '' ? null : Number(age.value); const result=await request('/api/preferences',{method:'POST',body:JSON.stringify({theme:theme.value,response_style:responseStyle.value,knowledge_level:knowledgeLevel.value,nickname:nickname.value,interests:interests.value,age:ageValue,occupation:occupation.value,gender_identity:genderIdentity.value,pronoun_mode:pronounMode.value,pronouns:pronouns.value,simple_language:simpleLanguage.checked,gendered_language:genderedLanguage.checked,allow_network_research:allowNetwork.checked,store_personalization:storePersonalization.checked})}); applyTheme(result.theme); preferencesState.textContent='Einstellungen gespeichert.'; preferencesState.className='success'; } catch(error) { preferencesState.textContent=error.message; preferencesState.className='error'; } finally { savePreferences.disabled=false; } });
accountCreate.addEventListener('click', () => accountAction('create')); accountLogin.addEventListener('click', () => accountAction('authenticate')); accountReset.addEventListener('click', () => resetPassword());
undo.addEventListener('click', async () => { if (!activeSession) return; undo.disabled=true; undoState.textContent='Änderung wird zurückgesetzt …'; try { const result=await request('/api/undo',{method:'POST',body:JSON.stringify({session_id:activeSession,step:1})}); undoState.textContent=result.restored ? `Rückgängig: Schritt ${esc(result.step)} wiederhergestellt.` : 'Rückgängig nicht möglich.'; undoState.className=result.restored ? 'success' : 'error'; undo.disabled = !result.remaining_undo; await refresh(); } catch(error) { undoState.textContent=error.message; undoState.className='error'; undo.disabled=false; } });
request('/api/health',{headers:{}}).then(data => { document.getElementById('health').textContent = data.status === 'ok' ? '● Runtime bereit' : 'Runtime prüfen'; }).catch(() => { document.getElementById('health').textContent='Runtime nicht erreichbar'; }); refresh(); refreshLearning(); refreshPreferences(); refreshAccount();
</script>
</body>
</html>"""
