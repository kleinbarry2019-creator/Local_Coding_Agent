...
print("=== SAFE AGENT V42 JSON NEWLINE FIX ===")

target = "safe_agent_v42.py"

V42_CODE = r'''#!/usr/bin/env python3
...
Antworte IMMER mit exakt EINEM JSON-Block. Verwende KEINEN Markdown-Text!

WICHTIGSTE REGEL: Echte Zeilenumbrüche in JSON-Strings sind VERBOTEN! 
Wenn du Code schreibst, musst du JEDEN Zeilenumbruch als '\\n' schreiben.
...
BEISPIEL 2: Code in Datei schreiben
{
  "action": "write_file",
  "file": "sysinfo.py",
  "content": "import os\\nprint('Hallo')\\n"
...
BEISPIEL 3: Fertig
{
  ...
  "result": "Erklärung"
    ...
    for step in range(max_steps):
        ...
        # LLM Auto-Fix: Repariert echte Zeilenumbrüche im "content"-Feld des JSON
        json_str = re.sub(r'("content"\s*:\s*")(.*?)("\s*\}|"\s*,)', lambda m: m.group(1) + m.group(2).replace('\n', '\\n').replace('\r', '') + m.group(3), json_str, flags=re.DOTALL)
        
        try:
            action_data = json.loads(json_str, strict=False)
        except json.JSONDecodeError as e:
            messages.append({"role": "user", "content": f"JSON Fehler: {e}. REGEL: Nutze IMMER \\n statt echter Zeilenumbrüche in Strings!"})
            continue
...
Path(target).write_text(V42_CODE, encoding="utf-8")
