...
echo "=========================================="
echo " SAFE AGENT V48 - BOUNDED PROTOCOL"
echo "=========================================="
echo "Project: $ROOT"
...
def main():
    if len(sys.argv) < 2:
        print(
            'Usage: python3 safe_agent_v48.py "AUFGABE"'
        )
        raise SystemExit(2)
...
def test_v48_model_response_limit():
    expect_blocked(
        "Model Response Limit",
...
def test_v48_network_isolation():
    name = "v48_network_probe.py"
...
def test_v48_symlink_protection():
    target = RUNTIME / "v48_link"
...
cat > tools/test_all_v48.sh <<'BASH2'
#!/usr/bin/env bash
set -Eeuo pipefail
...
python3 -m py_compile safe_agent_v48.py

python3 tests/test_v48.py
...
echo
echo "V48 TESTS PASSED"
BASH2

chmod +x tools/test_all_v48.sh


cat > tools/test_end_to_end_v48.sh <<'BASH3'
#!/usr/bin/env bash
set -Eeuo pipefail
    ...
    "messages": [
        {
            ...
            "content": (
                'Antworte exakt mit '
                '{"action":"done","result":"OLLAMA_V48_OK"}'
            ),
        }
    ],
    "stream": False,
...
if "OLLAMA_V48_OK" not in content:
    raise SystemExit(
        "OLLAMA FAIL"
    )
PY
...
python3 safe_agent_v48.py \
    "Erstelle v46_agent_test.txt mit exakt V48_AGENT_OK. Lies die Datei danach wieder ein. Prüfe den Inhalt und antworte erst dann mit done."
...
test -f safe_agent_v48.py

echo "AGENT CODE PRESENT"

echo
echo "V48 END-TO-END TESTS PASSED"
BASH3

chmod +x tools/test_end_to_end_v48.sh


cat > BUILD_INFO_V48.txt <<EOF
SAFE AGENT V48
...
echo
echo "=== V48 END-TO-END ==="
./tools/test_end_to_end_v48.sh

echo
echo "=========================================="
echo " SAFE AGENT V48 READY"
echo "=========================================="
