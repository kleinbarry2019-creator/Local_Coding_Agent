...
def policy_check(name, arguments):
    ...
    if name in {"getcwd", "list_files"}:
        if arguments:
            return False, f"{name} akzeptiert keine Argumente."
...
- Niemals ein bereits ausgeführtes Tool erneut anfordern.
- Fordere pro Antwort höchstens EIN Tool an.
- Wenn die Aufgabe vollständig erledigt ist, verwende {"final":"..."}.
"""
...
def main():
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "agent_v13_test.txt mit dem Inhalt "
                "\"V13 SEQUENTIAL OK\". "
                "Führe jedes Tool höchstens einmal aus. "
                "Wenn alles erledigt ist, gib eine kurze finale Zusammenfassung."
            ),
        }
    ]
    ...
    for step in range(1, MAX_MODEL_STEPS + 1):
        ...
        if parsed["type"] == "final":
            ...
            print("\nUsed tools:", sorted(used_tools))
            print("Tool calls:", len(history))
            print("\nSAFE AGENT V13 OK")
            return
