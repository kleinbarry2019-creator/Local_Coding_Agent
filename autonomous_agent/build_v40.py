...
print("=== SAFE AGENT V40 QUOTE FIX ===")

target = "safe_agent_v40.py"

V40_CODE = r'''#!/usr/bin/env python3
...
WICHTIGE REGELN FÜR JSON & BASH:
1. In JSON dürfen einfache Anführungszeichen (') NICHT mit Backslash escaped werden! `\\'` ist streng verboten. Nutze einfach `'`.
2. Verwende NIEMALS komplizierte `echo` Befehle, um Skripte zu schreiben (Escaping-Hölle!).
3. Nutze IMMER den Heredoc-Ansatz `cat << 'EOF' > datei.py` ... `EOF`, um Code sicher in Dateien zu schreiben.
...
Aktion 2: Fertig
{
  ...
  "result": "Lösung"
    ...
    for step in range(max_steps):
        ...
        # LLM Auto-Fix: Entfernt unerlaubte Escape-Sequenzen bei einfachen Anführungszeichen
        ...
        try:
            ...
            messages.append({"role": "user", "content": f"JSON Parse Fehler: {e}. REGEL: In JSON darfst du einfache Anführungszeichen (') NICHT escapen! Schreibe niemals \\', sondern immer nur '. Nutze für Dateierstellung immer cat << 'EOF' > datei statt echo."})
            continue
...
Path(target).write_text(V40_CODE, encoding="utf-8")
