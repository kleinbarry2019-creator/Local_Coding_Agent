...
echo "=========================================="
echo " SAFE AGENT V49 - BOUNDED PROTOCOL"
echo "=========================================="
echo "Project: $ROOT"
...
def main():
    if len(sys.argv) < 2:
        print(
            'Usage: python3 safe_agent_v49.py "AUFGABE"'
        )
        raise SystemExit(2)
...
def test_v49_version():
    assert agent.VERSION == 47


def test_v49_model_response_limit():
    expect_blocked(
        "Model Response Limit",
...
def test_v49_network_isolation():
    name = "v49_network_probe.py"
...
def test_v49_symlink_protection():
    target = RUNTIME / "v49_link"
...
if __name__ == "__main__":
    tests = [
        ...
        test_v49_model_response_limit,
        test_v49_list_files_limit,
        test_v49_path_length_limit,
        test_v49_content_limit,
        test_v49_symlink_protection,
        test_v49_network_isolation,    ]
...
cat > tools/test_all_v49.sh <<'BASH2'
#!/usr/bin/env bash
set -Eeuo pipefail
...
chmod +x tools/test_all_v49.sh


cat > tools/test_end_to_end_v49.sh <<'BASH3'
#!/usr/bin/env bash
set -Eeuo pipefail
    ...
    "messages": [
        {
            ...
            "content": (
                'Antworte exakt mit '
                '{"action":"done","result":"OLLAMA_V49_OK"}'
            ),
        }
    ],
    "stream": False,
...
if "OLLAMA_V49_OK" not in content:
    raise SystemExit(
        "OLLAMA FAIL"
    )
PY
...
rm -f runtime/v49_agent_test.txt

python3 safe_agent_v49.py \
    "Erstelle v49_agent_test.txt mit exakt V49_AGENT_OK. Lies die Datei danach wieder ein. Prüfe den Inhalt und antworte erst dann mit done."

test -f runtime/v49_agent_test.txt

test "$(
    cat runtime/v49_agent_test.txt
)" = "V49_AGENT_OK"
...
test -f safe_agent_v49.py

echo "AGENT CODE PRESENT"

echo
echo "V49 END-TO-END TESTS PASSED"
BASH3

chmod +x tools/test_end_to_end_v49.sh


cat > BUILD_INFO_V49.txt <<EOF
SAFE AGENT V49

Release cleanup:
- explicit V49 test coverage
...
echo
echo "=== V49 END-TO-END ==="
./tools/test_end_to_end_v49.sh

echo
echo "=========================================="
echo " SAFE AGENT V49 READY"
echo "=========================================="
